...
class KdbxFile
{
     ...
     *
     * @return KdbxHeader A KdbxHeader instance.
     */
     ...
     *
     * @return RandomStream An RandomStream instance.
     */
     ...
     *
     * @return bool if everything went well and the encrypt method can be
    ...
    public function prepareEncrypting(&$error)
    {
        ...
        if ($header->randomStream == KdbxHeader::RANDOMSTREAM_SALSA20) {
            $this->_randomStream = Salsa20RandomStream::create(
     ...
     * @param Key    $key     A Key instance to use as encryption key.
     * @param string &$error  A string that will receive a message in case of error.
     *
     * @return string|null A binary string containing the encrypted Kdbx file, or null in
     *                     case of error.
     */
    public function encrypt($content, Key $key, &$error)
    {
        if (empty($content)) {
     ...
     * @param string &$error A string that will receive a message in case of error.
     *
     * @return KdbxFile A new KdbxFile instance ready to be encrypted.
     */
     ...
     * @param Key    $key    The encryption key of the Kdbx file.
     * @param string &$error A string that will receive a message in case of error.
     *
     * @return KdbxFile A new KdbxFile instance containing the decrypted content,
     *                  header and random stream (if applicable), or null if something
     *                  went wrong.
     */
    public static function decrypt(Reader $reader, Key $key, &$error)
    {
        if ($reader == null) {
     ...
     *
     * @return bool if the header is valid, false otherwise.
     */
