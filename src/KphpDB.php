...
class KphpDB
{
     ...
     *
     * @param string $dbType      The wrapped database type.
     * @param string $db          An object that will be wrapped by this KphpDB.
     * @param string $dbFileHash  The hexadecimal hash of the original db file.
     * @param string $keyFileHash A possible hexadecimal hash of an additional key file if needed to decrypt the original db file.
     */
     ...
     *
     * @param string $db          A Database instance.
     * @param string $dbFileHash  The hexadecimal hash of the original db file.
     * @param string $keyFileHash A possible hexadecimal hash of an additional key file if needed to decrypt the original db file.
     *
     * @return static A new KphpDB instance.
     */
     ...
     *
     * @return string One of the KphpDB::DBTYPE_* constants.
     */
     ...
     *
     * @param Filter $filter An iFilter instance to select the data of the database
     *                       that must actually be serialized (if null, it will
     *                       serialize everything except from passowrds).
     * @param mixed  &$error A string that will receive a message in case of error.
     *
     * @return mixed A JSon string in case of success, or null in case of error.
     */
     ...
     * file.
     *
     * @param Key    $key    A iKey instance to use to encrypt the kdbx file.
     * @param Filter $filter An iFilter instance to select the data of the database
     *                       that must actually be serialized (if null, it will
     *                       serialize everything except from passowrds).
     * @param mixed  &$error A string that will receive a message in case of error.
     *
     * @return mixed A string containing a kdbx file embbeding this serialized
     *               instance, or null in case of error.
     */
    public function toKdbx(Key $key, $filter, &$error)
    {
     ...
     *
     * @return static A new KphpDB instance, or null in case of error.
     */
    public static function loadFromJSon($json, &$error)
    {
        ...
        } elseif ($dbType != self::DBTYPE_NONE) {
        ...
        $kdbx = new KphpDB(
            $dbType,
            $db,
            $array[self::KEY_DBFILEHASH],
            $array[self::KEY_KEYFILEHASH]
        );
     ...
     * @param Key    $key    A iKey instance to use to decrypt the kdbx file.
     * @param string &$error A string that will receive a message in case of error.
     *
     * @return static A new KphpDB instance, or null in case of error.
     */
